...
||Обратите внимание, пайпы `|>` и `%>%` имеют одну и ту же функцию. Но `%>%` придумали создатели пакета `magrittr`, входящего в `tidyverse`, а `|>` было введено как более интуитивно понятное обозначение оператора в базовый R в поздних версиях (начиная с 4.1.0).
