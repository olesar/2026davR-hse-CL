...
diet <- diet |>
  ...
  drop_na() # перекодируем участников и тип диеты как факторы, создадим столбец weight.loss
...
Посмотрим на средние в группах:

```{r}
diet |> 
  group_by(Dietf) |>
  rstatix::get_summary_stats(weight.loss, type = "mean_sd")
```
...
aov_model
summary(aov_model) # более полный вывод с p-values

# post-hoc 
TukeyHSD(aov_model)
```
Был проведен дисперсионный анализ (one-way ANOVA), который показал, что есть 
статистически значимая разница в средних в зависимости от типа диеты (F(2)=5.383, p=0.007).

или:
Был проведен дисперсионный анализ (one-way ANOVA), который показал статистически значимый эффект типа диеты (F(2)=5.383, p=0.007).
...
-   Мы применили однофакторный дисперсионный анализ с повторными
    измерениями, который показал (F(1,75)=188.471, p-value=3.72e-22),
...
```
Был проведен дисперсионный анализ с повторными измерениями эффекта времени на вес участников эксперимента, выбравших тип диеты C. Он показал статистически значимый эффект времени F(1,26)=124.6949, p=2.030459e-11), нулевую гипотезу о равенстве средних можно отвергнуть. Маловероятно, что разница объясняется дисперсией (ges=9,9%).
```
...
Шпаргалка по ANOVA:
* https://github.com/olesar/2026davR-hse-CL/blob/main/papers/r_coding_cheat_sheet.pdf  
